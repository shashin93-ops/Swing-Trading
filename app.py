...
st.title("📈 Swing Breakout Scanner — Nifty 500 (Weekly Timeframe)")
st.caption(
    "Weekly uptrend + daily consolidation + proximity to breakout + weekly volume "
    "expansion, to reduce false breakouts. Run this every Friday/Saturday after close."
)
...
if run_button:
    ...
    st.subheader(f"✅ Strict Breakout List ({len(strict_df)} stocks)")
    st.caption("Near/at breakout, confirmed by weekly volume expansion. Highest conviction.")
    st.dataframe(strict_df, use_container_width=True, hide_index=True)

    st.subheader(f"👀 Watchlist ({len(watchlist_df)} stocks)")
    st.caption("Setting up but not yet meeting the strict criteria — track these for next week.")
